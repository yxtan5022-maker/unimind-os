...
**最大绝对偏差 `|<Z>_经验 - <Z>_理论| = 0.0110 < 0.05`（通过）。** 实验使用固定模拟器种子（`seed_simulator=42`），因此上表数字跨运行完全可复现。经验测量概率在二项采样方差范围内收敛于理论权重，证实了经典预处理流水线能正确初始化量子态振幅。
...
> 注：表 2 包含 Python 侧 FFI 编组开销（逐元素归一化 + ctypes 缓冲区构建）；纯 Rust 内核还要更快，因此这些加速比是保守的端到端下限。
